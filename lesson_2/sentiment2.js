...
function sentiment2(text) {
  let positivesFound = text.match(positiveRegex).map(word => word.toLowerCase());
  let negativesFound = text.match(negativeRegex).map(word => word.toLowerCase());
  let sentiment;

  if (positivesFound.length > negativesFound.length) {
    sentiment = 'Positive';
  } else if (positivesFound.length < negativesFound.length) {
    sentiment = 'Negative';
  } else {
    sentiment = 'Neutral';
  }

  console.log(`There are ${positivesFound.length} positive words in the text.\n\
    Positive sentiments: ${positivesFound.join(', ')}\n`);
  
  console.log(`There are ${negativesFound.length} negative words in the text.\n\
    Negative sentiments: ${negativesFound.join(', ')}\n`);
